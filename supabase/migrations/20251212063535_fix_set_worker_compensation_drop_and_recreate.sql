/*
  ...
  The user_id column in worker_pay_rates is a generated column (computed from worker_id).
  This migration drops and recreates the function to not insert into user_id directly.
*/
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE farm_id = p_farm_id AND user_id = p_worker_id AND is_active = true
  ) THEN
