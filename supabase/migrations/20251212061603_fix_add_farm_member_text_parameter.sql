/*
  # Fix add_or_update_farm_member to Accept Text Parameter

  1. Changes
    - Drop old function with enum parameter
    - Recreate with text parameter and internal casting
    - This fixes PostgREST schema cache issues with enum types

  2. Notes
    - The function now accepts role as text and casts internally
*/

DROP FUNCTION IF EXISTS add_or_update_farm_member(uuid, text, membership_role);
...
BEGIN
  ...
  FROM farm_members
  WHERE farm_id = p_farm_id AND user_id = v_caller_id AND status = 'active';
  ...
  IF v_existing_member_id IS NOT NULL THEN
    UPDATE farm_members
    SET role = v_new_role, status = 'active', updated_at = now()
  ...
  ELSE
    ...
    VALUES (p_farm_id, v_target_user_id, v_new_role, 'active')
