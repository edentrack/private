...
export function FarmJoinCodeSection() {
  ...
  const refreshLink = async () => {
    ...
    await fetchSecret();
    setRefreshing(false);
    showToast(isFr ? 'Nouveau lien généré' : 'Fresh link generated', 'success');
  };
  ...
  const copyLink = async () => {
    ...
    showToast(isFr ? "Lien d'invitation copié !" : 'Join link copied!', 'success');
  };
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: isFr ? `Rejoindre ${currentFarm.name} sur Edentrack` : `Join ${currentFarm.name} on Edentrack`,
          text,
          url: joinLink,
        });
        return;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl p-3">
        ...
        <div className="flex gap-1.5 shrink-0">
          <button
            ...
            title={isFr ? 'Générer un nouveau lien' : 'Generate a new link'}
          >
      ...
      <p className="text-xs text-gray-400">
        {isFr ? (
          <>
            Tout le monde rejoint en tant qu'<strong>ouvrier</strong>. Promouvez en gestionnaire dans la gestion d'équipe ensuite.
            Utilisez le bouton <RefreshCw className="inline w-3 h-3" /> pour générer un nouveau lien à tout moment.
          </>
        ) : (
          <>
