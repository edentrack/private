...
import { Plus, Syringe, Check, Calendar } from 'lucide-react';
      ...
      {!currentFlock ? (
        ...
        <>
          ...
          {vaccinations.length === 0 && !showAddForm && (
            <div className="bg-white rounded-3xl p-12 text-center">
              <Syringe className="w-16 h-16 text-gray-400 mx-auto mb-4" />
              <h3 className="text-xl font-bold text-gray-900 mb-2">{t('vaccinations.no_vaccinations_scheduled')}</h3>
              <p className="text-gray-600 mb-6">{t('vaccinations.add_first_vaccination')}</p>
            </div>
          )}
        </>
      )}
